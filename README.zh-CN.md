...
`alembic_codex_bootstrap` 和 `alembic_codex_rescan` 是显式 provider-backed Alembic daemon job。它们需要已配置 AI Provider 凭据，并会立即返回持久 job id。Codex 重连或本地 Alembic UI 刷新后，用 `alembic_codex_job` 携带该 id 继续检查状态。
