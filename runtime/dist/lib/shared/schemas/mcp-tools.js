 ...
 */
import { ComplexityEnum, ContentSchema, IdField, KindEnum, LanguageField, ReasoningSchema, ScopeEnum, StrictKindEnum, TitleField, } from '@alembic/core/shared';
import { z } from 'zod';
// ══════════════════════════════════════════════════════
...
}).describe('Agent-facing work start. Creates a workRef for concrete implementation/fix/refactor/review work without loading knowledge, running Guard, or calling legacy task operations.');
export const WorkFinishInput = AgentPublicToolBaseInput.extend({
    ...
    changedFiles: AgentSourceFileRefsInput.describe('Task-scoped files changed by this work; used to recommend alembic_code_guard with explicit files.'),
    evidenceRefs: AgentSourceFileRefsInput.describe('Non-private evidence refs from build/test/logs'),
    validationPlan: z
        .record(z.string(), z.unknown())
        .optional()
        .describe('Optional compact output from alembic_validation_plan. Buckets are advisory and do not replace Guard, repository tests, controller acceptance, or Test-window validation.'),
    reason: z.string().min(1).max(1200).optional().describe('Blocked or abandoned reason'),
}).describe('Agent-facing work finish. Returns finishRef, detailRefs, and scoped Guard recommendation metadata; it does not run Guard.');
export const CodeGuardInput = AgentPublicToolBaseInput.extend({
    intentRef: AgentRefIdInput.optional().describe('intentRef returned by alembic_intent'),
    workRef: AgentRefIdInput.optional().describe('workRef returned by alembic_work_start. When files/code are omitted, the current session work record supplies scoped files; missing or unscoped work returns a structured blocker/skip.'),
    files: AgentSourceFileRefsInput.describe('Explicit files to check. This is the preferred scope. Empty or omitted scope returns a structured blocker/skip instead of falling back to whole-diff review.'),
...
export const DecisionRecordInput = AgentPublicToolBaseInput.extend({
    ...
    decisionRef: AgentRefIdInput.optional().describe('Existing decisionRef for update/revoke/delete/read'),
    intentRef: AgentRefIdInput.optional().describe('intentRef returned by alembic_intent'),
    workRef: AgentRefIdInput.optional().describe('workRef returned by alembic_work_start'),
...
}).describe('Agent-facing decision record. Uses a durable Decision Register route when available; otherwise returns a structured blocker rather than local fake persistence.');
// ══════════════════════════════════════════════════════
//  2. alembic_search
// ══════════════════════════════════════════════════════
...
export const GuardInput = z.object({
    operation: z
        ...
        .optional()
        .describe('Guard 操作类型。coverage_matrix: 模块覆盖率矩阵；compliance_report: 3D 合规报告（含 uncertain）。省略且缺少 files/code 时返回 missing-guard-scope blocker，不再自动扫描整个 git diff。'),
...
export const TaskInput = z.object({
    operation: z
        ...
        .describe('Legacy compatibility operation. Prefer public tools: alembic_intent/alembic_prime, alembic_work_start, alembic_work_finish, alembic_code_guard, and alembic_decision_record.'),
    ...
    changedFiles: z
        ...
        .max(50)
        .optional()
        .describe('Task-scoped files changed by this legacy task; compatibility close uses them to recommend explicit alembic_code_guard scope.'),
    sourceRefs: z
        .array(z.string().min(1).max(200))
        .max(50)
        .optional()
        .describe('Optional non-private source refs for legacy task-scope evidence and public-tool detailRefs.'),
});
// ══════════════════════════════════════════════════════
//  Admin Tools
// ══════════════════════════════════════════════════════
...
export const KnowledgeLifecycleInput = z.object({
    ...
    action: z
        ...
        .enum(['reactivate'])
        .describe('reactivate=请求将 deprecated Recipe 恢复为 pending review；publish/deprecate/approve/fast_track 不对默认 Codex agent 开放，请走 Dashboard 或 admin 路径'),
    reason: z
        .string()
        .optional()
