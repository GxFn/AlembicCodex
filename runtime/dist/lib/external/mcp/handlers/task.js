...
async function _prime(ctx, args) {
    ...
    if (primeKnowledgeMaterial.status === 'degraded') {
        ...
        lines.push('📣 Codex must immediately tell the developer prime degraded before any further tool call, code reading, edit, Guard check, or final summary.');
    }
        ...
        lines.push('📣 Codex must immediately shout a short knowledge receipt before any further tool call, code reading, edit, Guard check, or final summary. Summarize the accepted constraints, patterns, and guardrails; keep evidenceRefs in the payload for later verification instead of listing paths by default.');
    }
    else {
        lines.push('No matching recipes found.');
        lines.push('📣 Codex must immediately tell the developer prime returned no usable project knowledge before any further tool call, code reading, edit, Guard check, or final summary.');
    }
    return envelope({
        success: true,
        data: {
...
function _buildPrimeShoutInstruction(status) {
    if (status === 'delivered') {
        return [
