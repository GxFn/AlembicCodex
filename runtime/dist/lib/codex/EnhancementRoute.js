...
export function summarizeEnhancementDaemon(status) {
    ...
    const capabilitySummary = mergeCapabilitySummaryWithRuntimeBoundary(mergeCapabilitySummaryWithResidentService(summarizeAlembicRuntimeCapabilities(capabilities), residentServiceStatus), runtimeBoundary);
    const route = firstString(residentServiceStatus?.route, enhancement?.route, runtimeBoundary.route) ||
        inferRouteFromReadyDaemon(status);
    const dashboardUrl = firstString(capabilitySummary.dashboardUrl, runtimeBoundary.dashboard.url, data?.dashboardUrl, isLocalAlembicDaemonRoute(route) && capabilitySummary.dashboardAvailable === true
...
function mergeCapabilitySummaryWithResidentService(summary, residentService) {
    ...
    const fileMonitorAvailable = available('file-monitor.git-worktree')
        ...
        : unavailable('file-monitor.git-worktree')
            ? false
            : null;
    const internalAiAvailable = available('jobs.internal-ai.bootstrap') || available('jobs.internal-ai.rescan')
        ? true
