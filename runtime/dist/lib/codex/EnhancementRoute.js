...
import { normalizeAlembicFileMonitorMode, normalizeAlembicResidentServiceStatus, summarizeAlembicResidentServiceStatus, summarizeAlembicRuntimeCapabilities, } from '@alembic/core/daemon';
...
export function probeLocalAlembicInstall(command = 'alembic') {
...
export function summarizeEnhancementDaemon(status) {
    ...
    const capabilitySummary = mergeCapabilitySummaryWithResidentService(toCodexDaemonCapabilitySummary(summarizeAlembicRuntimeCapabilities(capabilities)), residentServiceStatus);
    const route = firstString(residentServiceStatus?.route, enhancement?.route) ||
        inferRouteFromReadyDaemon(status);
    const dashboardUrl = firstString(capabilitySummary.dashboardUrl, data?.dashboardUrl, isLocalAlembicDaemonRoute(route) && capabilitySummary.dashboardAvailable === true
    ...
    return {
        ...
        compatibility: {
            runtimeBoundary: summarizeRuntimeBoundaryCompatibility(runtimeBoundary, residentServiceStatus),
        },
        dashboardUrl,
...
function summarizeRuntimeBoundaryCompatibility(runtimeBoundary, residentService) {
...
function mergeCapabilitySummaryWithResidentService(summary, residentService) {
    if (!residentService) {
        return summary;
    }
    const available = (feature) => residentService.capabilities[feature]?.available === true;
    const unavailable = (feature) => residentService.capabilities[feature]?.available === false;
    const jobKinds = [
    ...
    const fileMonitorAvailable = available('file-monitor.git-worktree')
        ...
        : unavailable('file-monitor.git-worktree')
            ? false
            : null;
    const residentDaemonJobsAvailable = available('jobs.api-ai.bootstrap') || available('jobs.api-ai.rescan')
        ? true
    ...
    const statusAvailable = available('status.health')
        ...
        : unavailable('status.health')
            ? false
            : null;
    const jobsAvailable = jobKinds.length > 0
        ? true
...
function toCodexDaemonCapabilitySummary(summary) {
    return {
...
function findMissingCapabilities(requirement, daemon) {
    ...
    if (requirement === 'jobs') {
        const hasJobs = daemon.capabilities.jobsAvailable !== false &&
...
function summarizeResidentDaemonJobProvider(health) {
...
function inferRouteFromReadyDaemon(status) {
    if (!status.ready) {
        return null;
    }
    return 'embedded-plugin-runtime';
