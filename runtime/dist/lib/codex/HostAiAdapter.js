...
export class HostAiProviderManager {
    ...
    #syncToDi = null;
    #tokenRecorder = null;
    constructor(initialProvider = null) {
...
export function createHostAiProviderManager(initialProvider = null) {
    return new HostAiProviderManager(initialProvider);
}
export function createHostManagedProvider(options) {
    const provider = normalizeProviderName(options.provider || process.env.ALEMBIC_AI_PROVIDER) || 'mock';
    return {
        name: provider,
...
export function readHostAiConfigInfo(projectRoot) {
    const state = inspectCodexAiConfig(projectRoot);
    const keys = Object.fromEntries(Object.entries(PROVIDER_KEY_ENV).map(([provider, envKey]) => [
        provider,
...
export function listHostAiProviders(options = {}) {
...
function normalizeHostProvider(provider) {
    if (!provider || typeof provider !== 'object') {
        return createHostManagedProvider({ provider: 'mock', model: 'mock-fallback' });
    }
    const providerName = normalizeProviderName(provider.name || provider.provider || provider.type) || 'mock';
    const model = typeof provider.model === 'string' && provider.model.length > 0
        ? provider.model
...
function defaultModelForProvider(provider) {
    return (HOST_PROVIDER_CONFIGS.find((entry) => entry.id === provider)?.defaultModel ||
        (provider === 'mock' ? 'mock-l3' : 'host-managed-model'));
}
function hasChatCapability(provider) {
...
function unavailableProviderMethods() {
    const unavailable = () => {
        throw new Error('AI execution is provided by the host agent and is not bundled in AlembicPlugin.');
    };
    return {
