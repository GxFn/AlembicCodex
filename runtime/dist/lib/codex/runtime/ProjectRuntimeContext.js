...
import { createProjectRuntimeFailureEnvelope, createProjectRuntimeIdentityContract, createProjectRuntimeServiceReadiness, normalizeAlembicRuntimeDataRootSource, } from '@alembic/core/daemon';
...
function buildProjectRuntimeIdentity(input) {
    ...
    const residentDataRoot = residentIdentity?.available === true
        ? (stringFrom(residentIdentity.dataRoot) ?? stringFrom(residentScope?.dataRoot))
        : null;
    if (residentDataRoot) {
        const runtimeDir = join(residentDataRoot, '.asd');
        const dataRootSource = normalizeAlembicRuntimeDataRootSource(residentIdentity?.dataRootSource) ??
        ...
        return createProjectRuntimeIdentityContract({
            ...
            projectScopeId: residentIdentity?.projectScopeId ??
...
function buildRequiredServiceReadiness(input) {
    ...
    const services = input.includeOptionalServices
        ? ['project-identity', 'project-scope', 'daemon', 'jobs', 'api-ai', 'dashboard', 'file-monitor']
        : [...required];
    return services.map((service) => createProjectRuntimeServiceReadiness({
...
function isServiceAvailable(service, input) {
    switch (service) {
        case 'project-identity':
            return Boolean(input.identity.projectRoot &&
        ...
        case 'dashboard':
            return (input.enhancementRoute?.selected === 'local-alembic-daemon' &&
...
function serviceFailureReason(service, input) {
...
function serviceMessage(service, input) {
    ...
    if (service === 'daemon') {
        return (input.sourceOfTruth?.failure?.blockingCondition?.toString() ??
...
function daemonFailureReason(status, sourceOfTruth) {
...
function summarizeReadiness(services) {
...
function detectCodexMcpEntryMode(runtime) {
    ...
    const mode = args.some((arg) => arg.endsWith('alembic-codex-mcp-wrapper.mjs'))
        ? 'packaged-wrapper'
        : args.some((arg) => arg.endsWith('/dist/bin/codex-mcp.js') || arg.endsWith('dist/bin/codex-mcp.js'))
...
function extractAlembicRuntimeSourceOfTruth(daemonStatus) {
