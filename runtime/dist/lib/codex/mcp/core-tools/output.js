...
export const CORE_TOOL_OUTPUT_SCHEMAS = Object.fromEntries(CORE_CLEAN_OUTPUT_TOOL_NAMES.map((toolName) => [
    toolName,
    createCoreToolOutputSchema(toolName, CORE_TOOL_ALLOWED_BUSINESS_FIELD_NAMES[toolName]),
]));
export function projectCoreToolOutput(input, toolName) {
...
export function findForbiddenCoreOutputField(value, path = []) {
...
function createCoreToolOutputSchema(toolName, businessFields) {
...
function sanitizeBusinessValue(value) {
    if (!value || typeof value !== 'object') {
        return value;
    }
    if (Array.isArray(value)) {
...
function pickAllowedBusinessFields(value, toolName) {
    const allowed = new Set(CORE_TOOL_ALLOWED_BUSINESS_FIELD_NAMES[toolName]);
    const out = {};
    for (const [key, child] of Object.entries(value)) {
        if (allowed.has(key)) {
            out[key] = child;
