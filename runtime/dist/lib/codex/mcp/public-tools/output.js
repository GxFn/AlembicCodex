...
export function createAgentPublicToolOutput(result, payload = {}, options = {}) {
...
function projectRecognizedIntentPublic(value) {
...
function projectGuardPublicResult(value) {
    const record = asRecord(value);
...
function projectDecisionSummary(value) {
    const record = asRecord(value);
    const decision = {
        ...(stringFrom(record.action) ? { action: stringFrom(record.action) } : {}),
...
function projectDurablePersistence(value) {
    const record = asRecord(value);
    return {
...
function projectDecisionCapability(value) {
    const record = asRecord(value);
...
function projectRequestedDecision(value) {
    const record = asRecord(value);
    return {
        action: isDecisionAction(record.action) ? record.action : 'create',
...
function stringFrom(value, max = 1200) {
    if (typeof value !== 'string') {
        return undefined;
    }
    return value.slice(0, max);
}
function stringArray(value, maxItems = 80, maxLength = 1200) {
