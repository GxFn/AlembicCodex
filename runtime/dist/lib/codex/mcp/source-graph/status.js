...
import { createSourceGraphAffectedTestsResult, createSourceGraphCalleesResult, createSourceGraphCallersResult, createSourceGraphExploreResult, createSourceGraphFreshness, createSourceGraphImpactResult, createSourceGraphNodeResult, createSourceGraphSearchResult, createSourceGraphStatusResult, createSourceGraphValidationPlanResult, SourceGraphRepositoryImpl, SourceGraphService, } from '@alembic/core/source-graph';
import { WorkspaceResolver } from '@alembic/core/workspace';
import { CODEX_LOCAL_TOOLS } from '../../ToolPolicy.js';
import { buildCodexMcpGuidance } from '../host/guidance.js';
import { projectSourceGraphOperationBusiness, SOURCE_GRAPH_OPERATION_TOOL_NAMES, } from './output.js';
const DEFAULT_REPO_ID = 'default';
const DEFAULT_MAX_CATCH_UP_FILES = 50;
const SOURCE_GRAPH_QUERY_TOOL_NAMES = SOURCE_GRAPH_OPERATION_TOOL_NAMES.filter((toolName) => toolName !== 'alembic_source_graph_status');
// Reuse Core source graph runtimes per project inside one Codex MCP session.
const SOURCE_GRAPH_RUNTIME_CACHE = new Map();
export async function buildSourceGraphStatus(projectRoot, args = {}) {
...
export async function buildSourceGraphOperation(projectRoot, args = {}, toolName) {
...
export async function buildFullSourceGraphIndexForProject(projectRoot, args = {}) {
    const options = normalizeSourceGraphStatusOptions(args);
...
export async function resetSourceGraphRuntimeCacheForTests() {
    await Promise.allSettled(Array.from(SOURCE_GRAPH_RUNTIME_CACHE.values(), async (runtimePromise) => {
        const runtime = await runtimePromise;
        runtime.connection.close();
    }));
    SOURCE_GRAPH_RUNTIME_CACHE.clear();
}
async function inspectSourceGraphOperation(projectRoot, options, toolName) {
    ...
    if (!resolution.databaseExists && !hasRuntime && options.catchUp === false) {
        return projectOperationResult(createUninitializedStatus(resolution.projectRoot, options), options, toolName);
    }
    try {
        const runtime = await getSourceGraphRuntime(resolution);
        const catchUp = !resolution.databaseExists && !hasRuntime
        ...
        return projectOperationResult(createUnavailableStatus(resolution.projectRoot, options, err), options, toolName);
    }
}
async function inspectSourceGraphStatus(projectRoot, options) {
    const resolution = resolveSourceGraphRuntime(projectRoot, options);
    if ('error' in resolution) {
...
async function queryCoreSourceGraph(runtime, status, options, toolName) {
    const ranking = {
...
function projectOperationResult(status, options, toolName) {
    return projectSourceGraphOperationBusiness(createBlockedOperationResult(status, options, toolName), toolName);
}
function createBlockedOperationResult(status, options, toolName) {
    const base = {
...
async function maybeCatchUpSourceGraph(runtime, report, options) {
...
async function inspectAndMaybeCatchUpSourceGraph(runtime, options) {
...
async function buildInitialSourceGraphIndex(runtime, options) {
    const result = await runtime.service.buildFullIndex({
        projectRoot: runtime.projectRoot,
        repoId: runtime.repoId,
        projectScope: runtime.coreProjectScope,
...
function projectRuntimeStatus(status, context = {}) {
...
function resolveSourceGraphRuntime(projectRoot, options) {
    ...
    if (!projectScope && isWorkspaceControlRoot(resolvedRoot)) {
        return {
            ...
            error: createWrongScopeStatus(resolvedRoot, options, {
                ...
                message: 'Select a product projectScope before reading source graph facts from a workspace root.',
            }),
        };
    }
...
function getSourceGraphRuntime(resolution) {
...
function hasCachedSourceGraphRuntime(resolution) {
    return SOURCE_GRAPH_RUNTIME_CACHE.has(sourceGraphRuntimeCacheKey(resolution));
}
function sourceGraphRuntimeCacheKey(resolution) {
    return [
...
async function openSourceGraphRuntime(resolution) {
    const connection = new DatabaseConnection({ path: resolution.resolver.databasePath }, resolution.resolver);
...
async function runMigrationsQuietly(connection) {
...
function createUninitializedStatus(projectRoot, options) {
...
function createWrongScopeStatus(projectRoot, options, detail = {}) {
    const freshness = createSourceGraphFreshness({
        status: 'wrong-scope',
        checkedAt: options.now,
...
function resolveProjectScopeRoot(projectRoot, projectScope) {
    if (!projectScope) {
        return null;
    }
...
function isWorkspaceControlRoot(projectRoot) {
    return (existsSync(path.join(projectRoot, 'workspace.config.json')) ||
...
function createUnavailableStatus(projectRoot, options, err) {
    const freshness = createSourceGraphFreshness({
        status: 'unavailable',
        checkedAt: options.now,
...
function createDegradedStatus(projectRoot, options, previous, err) {
    const freshness = createSourceGraphFreshness({
        status: 'degraded',
        checkedAt: options.now,
...
function createInitialIndexFailedStatus(projectRoot, options, previous, err) {
    const freshness = createSourceGraphFreshness({
        status: 'unavailable',
        checkedAt: options.now,
...
function buildSourceGraphInitializeGuidance() {
...
function isSourceGraphQueryToolName(toolName) {
    return SOURCE_GRAPH_QUERY_TOOL_NAMES.includes(toolName);
}
function canQuerySourceGraph(status) {
    return (Boolean(status.generationId) &&
        (status.freshness.status === 'fresh' ||
            status.freshness.status === 'partial' ||
            status.freshness.status === 'degraded'));
}
function sourceGraphActionsForFreshness(freshness) {
    switch (freshness) {
        case 'uninitialized':
            return [
                {
            ...
            return [
                {
                    ...
                    description: 'Use available source graph context with diagnostics; validate unsupported or degraded coverage with raw reads/tests.',
                },
            ];
        case 'wrong-scope':
            return [
...
function normalizeSourceGraphStatusOptions(args) {
    return {
        catchUp: typeof args.catchUp === 'boolean' ? args.catchUp : undefined,
        maxCatchUpFiles: typeof args.maxCatchUpFiles === 'number' && Number.isInteger(args.maxCatchUpFiles)
...
function normalizeSourceGraphOperationOptions(args) {
    return {
...
function normalizeStringRecord(value) {
    ...
    const entries = Object.entries(value).filter((entry) => typeof entry[0] === 'string' &&
        entry[0].length > 0 &&
