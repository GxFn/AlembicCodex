...
import { buildPrimeKnowledgeMaterial, createUnavailablePrimeIntentEpisodeMaterial, } from '#service/task/PrimeKnowledgeMaterial.js';
...
import { createAgentDetailRef, createAgentPublicToolOutput, createAgentPublicToolResultEnvelope, createPrimePublicPackage, PRIME_PUBLIC_TRUST_LAYERS, } from '../public-tools/index.js';
...
function buildPrimeBlockingOutput(input) {
...
function buildPrimeBlockingResult(input) {
...
function buildPrimeRefs(input) {
    return {
        ...
        ...(input.args.intentRef
            ...
            : {}),
        detailRefs: input.detailRefs,
        primeRef: { refType: 'prime', id: input.primeRef, toolName: 'alembic_prime' },
    };
}
function buildPrimeReadyOutput(input) {
...
function buildPrimeMaterialProjection(intake, primeSearch) {
    return {
        primeKnowledgeMaterial: buildPrimeKnowledgeMaterial({
            extracted: intake.extracted,
            hostIntentFrame: intake.hostIntentFrame,
            hostIntentInput: intake.hostIntentInput,
            intentEpisode: createUnavailablePrimeIntentEpisodeMaterial('agent-public-prime keeps IntentEpisode handoff out of Stage 3 active surface'),
...
function buildPrimeReadyResult(input, status) {
...
export async function workStartHandler(ctx, args) {
    const intake = buildIntentIntake(ctx, args);
    const detailRefs = buildBaseDetailRefs('alembic_work_start', uniqueStrings([
        ...(args.sourceRefs ?? []),
    ...
    const title = firstString(args.title, args.workScope?.goal, intake.hostIntentFrame.recognizedIntentDraft.query) ?? workRef;
    const effectiveProjectRoot = resolveEffectiveProjectRoot(ctx, args);
...
export async function workFinishHandler(ctx, args) {
    const intake = buildIntentIntake(ctx, args);
    const detailRefs = buildBaseDetailRefs('alembic_work_finish', uniqueStrings([
        ...(args.sourceRefs ?? []),
        ...(args.sourceGraphRef ? [args.sourceGraphRef] : []),
        ...(args.sourceEvidenceRefs ?? []),
        ...(args.evidenceRefs ?? []),
    ...
    const summary = firstString(args.summary, args.reason) ??
        (outcome === 'completed'
...
export async function codeGuardHandler(ctx, args) {
    const intake = buildIntentIntake(ctx, args);
    const detailRefs = buildBaseDetailRefs('alembic_code_guard', uniqueStrings([
        ...(args.sourceRefs ?? []),
        ...(args.sourceGraphRef ? [args.sourceGraphRef] : []),
        ...(args.sourceEvidenceRefs ?? []),
    ]));
...
function resolveCodeGuardScope(ctx, args) {
...
function buildMissingWorkRefGuardOutput(input) {
    ...
    const result = createAgentPublicToolResultEnvelope({
        ...
        status: 'blocked',
        summary: buildResultSummary('Code Guard blocked because the requested workRef is not active in this Plugin session.'),
...
function buildEmptyWorkRefGuardOutput(input, workRecord) {
    const { args, detailRefs, intake, scope } = input;
    const result = createAgentPublicToolResultEnvelope({
        actionKind: 'code-guard',
        agentHost: intake.agentHost,
        ...
        status: 'skipped',
        summary: buildResultSummary('Code Guard skipped because the workRef has no scoped source files.'),
        toolName: 'alembic_code_guard',
    });
    return createAgentPublicToolOutput(result, {
...
async function executeScopedCodeGuard(ctx, args, scope) {
    if (scope.hasCode) {
...
function buildCodeGuardReadyOutput(input) {
    ...
    const result = createAgentPublicToolResultEnvelope({
        ...
        status: 'ready',
        summary: buildResultSummary(scope.hasCode
...
function buildCodeGuardExplicitScope(args, scope) {
    if (scope.hasCode) {
...
export async function decisionRecordHandler(ctx, args) {
    const intake = buildIntentIntake(ctx, args);
    ...
    if (!client) {
        const result = buildDecisionRecordBlockedResult({
            ...
            detailRefs,
            intake,
            message: 'Decision Register durable persistence is not available in AlembicPlugin; residentDecisionRegisterClient is not registered.',
    ...
    if (!residentResult.ok) {
        ...
        const result = buildDecisionRecordBlockedResult({
            ...
            summary: reasonCode === 'decision-register-capability-mismatch'
    ...
    const result = createAgentPublicToolResultEnvelope({
        ...
        status: 'ready',
        summary: buildResultSummary(formatDecisionRecordSuccessSummary(action, residentResult.value, decisionId)),
        toolName: 'alembic_decision_record',
    });
    return createAgentPublicToolOutput(result, {
...
function buildIntentIntake(ctx, args) {
    const hostDeclaredIntent = mergeRecognizedIntent(args);
    const rawUserQuery = firstString(args.userQuery, args.query);
    ...
    return {
        agentHost: args.agentHost ?? 'codex',
        extracted,
        hostIntentFrame,
        hostIntentInput,
        inputSource,
...
function intakeFromRecord(record, ctx, args) {
    return {
        agentHost: args.agentHost ?? 'codex',
...
function mergeRecognizedIntent(args) {
    const recognized = args.recognizedIntent;
    ...
    if (lifecycle.inputSource === 'automation-envelope' || intentKind === 'mechanical-envelope') {
        return {
            reason: {
                ...
                message: 'Raw automation envelope detected without enough curated host intent for public intent intake.',
                retryable: false,
            },
            status: 'skipped',
...
function resolvePrimeBlockingReason(args, record, intake) {
    const hasFallbackIntent = Boolean(intake.hostIntentFrame.recognizedIntentDraft.query.trim() ||
    ...
    if (intake.inputSource === 'automation-envelope' && intake.sourceRefs.length === 0) {
        return {
            ...
            message: 'Automation-envelope prime requires explicit sourceRefs so the host can verify referenced dispatch/plan evidence.',
        };
    }
    return null;
}
async function runPrimeSearch(ctx, intake, effectiveProjectRoot) {
    ...
    try {
        ...
        process.stderr.write(`[MCP/AgentPublicTools] alembic_prime search degraded: ${err instanceof Error ? err.message : String(err)}\n`);
        return { searchDegraded: true, searchResult: null, skippedReason: null };
    }
}
    ...
    if (input.searchDegraded) {
        return {
            reason: {
                ...
                message: 'Prime search degraded because the search pipeline or resident route was unavailable.',
                retryable: true,
            },
            status: 'degraded',
    ...
    if (relatedCount === 0 && guardCount === 0) {
        return {
            reason: {
                ...
                message: 'Prime completed structure-first retrieval but found no matching Recipe or Guard knowledge.',
                retryable: true,
            },
            status: 'degraded',
    ...
    if (intake.inputSource === 'automation-envelope' && intake.sourceRefs.length === 0) {
        return {
            reason: {
                kind: 'skip',
                code: 'mechanical-envelope-only',
                message: 'Raw automation envelope work start requires curated hostDeclaredIntent and sourceRefs.',
                retryable: false,
            },
            status: 'skipped',
...
function buildGuardRecommendation(decision, sourceGraph) {
...
function projectValidationPlanAdvisory(value, options = {}) {
...
function projectValidationBucket(value) {
    const recommendations = Array.isArray(value) ? value.filter(isRecord) : [];
    return {
        commands: uniqueStrings(recommendations.flatMap((item) => validationCommandRefs(item))).slice(0, 20),
        count: Math.min(recommendations.length, 1000),
        diagnosticCodes: uniqueStrings(recommendations.flatMap((item) => validationDiagnosticRefs(item))).slice(0, 20),
        files: uniqueStrings(recommendations.flatMap((item) => validationFileRefs(item))).slice(0, 40),
    };
}
function asValidationPlanSource(value) {
    const record = isRecord(value) ? value : {};
    if (isRecord(record.validationPlan)) {
        return record.validationPlan;
    }
    if (Array.isArray(record.mustRun) ||
...
function validationCommandRefs(item) {
    return [firstString(item.command)].filter((entry) => Boolean(entry));
}
function validationDiagnosticRefs(item) {
...
function validationFileRefs(item) {
    const evidence = Array.isArray(item.evidence) ? item.evidence.filter(isRecord) : [];
    return [
...
function projectGuardBusinessPayload(guardEnvelope) {
    if (!guardEnvelope || typeof guardEnvelope !== 'object') {
        return { guardResult: guardEnvelope };
    }
    const record = guardEnvelope;
    return {
...
function buildDecisionRecordRefs(args, detailRefs, decisionId) {
    return {
        ...buildSourceGraphRefEntry(args.sourceGraphRef),
        ...(args.intentRef
            ? {
...
function buildSourceGraphRefEntry(sourceGraphRef) {
...
function buildIntentRefEntry(intentRef) {
...
function buildWorkRefEntry(workRef) {
...
function resolveResidentDecisionRegisterClient(container) {
...
function tryGetContainerService(container, name) {
    try {
...
function isResidentDecisionRegisterClientLike(value) {
    return Boolean(value &&
        typeof value === 'object' &&
        typeof value.decisionRegister === 'function');
}
...
function buildRequestedDecision(action, args) {
    return {
        action,
...
function formatDecisionRecordSuccessSummary(action, result, decisionId) {
    if (action === 'list') {
...
function hasDecisionUpdatePayload(args) {
    return Boolean(firstString(args.title, args.description, args.rationale, args.intentRef, args.workRef) ||
        (args.tags?.length ?? 0) > 0 ||
        (args.evidenceRefs?.length ?? 0) > 0 ||
        (args.sourceRefs?.length ?? 0) > 0);
}
function truncateDecisionTitle(value) {
    if (!value) {
        return undefined;
    }
...
function bindPrimeSessionIntent(ctx, intake, searchResult, projectRuntime) {
    if (!ctx.session) {
        return;
    }
...
function bindWorkSession(ctx, record, intake) {
    if (!ctx.session) {
        return;
    }
...
function rememberWorkRecord(record) {
    WORK_RECORDS.set(record.workRef, record);
    if (WORK_RECORDS.size <= 100) {
        return;
    }
    const oldest = [...WORK_RECORDS.entries()].sort((left, right) => new Date(left[1].createdAt).getTime() - new Date(right[1].createdAt).getTime())[0]?.[0];
    if (oldest) {
...
function buildIntentClassification(intake, persistence, vectorPlan) {
    return {
...
function buildIntentPersistenceReceipt(persistence) {
    return {
...
function buildIntentRetrievalPlan(vectorPlan) {
    return {
        route: 'structure-first',
        vectorUseKind: vectorPlan.vectorUseKind,
    };
}
function buildIntentToolPlan(intake, persistence) {
...
function resolveKnowledgeNeed(primeNeed) {
    return primeNeed;
}
function resolveSourceGraphNeed(intake, persistence) {
    if (!persistence.consumable) {
        return 'none';
    }
    ...
    if (intake.intentKind === 'read-only-analysis' &&
        (intake.hostIntentInput.activeFile || intake.extracted.module)) {
        return 'optional';
    }
    return 'none';
}
function buildSourceGraphPlan(intake, persistence) {
    ...
    const changedFileLikely = intake.intentKind === 'implementation-task' ||
        intake.intentKind === 'fix-task' ||
        intake.intentKind === 'refactor-task';
    return {
        action: changedFileLikely
            ? 'validation-plan-after-work'
            : 'query-before-work',
...
function resolveDecisionNeed(intake) {
...
function buildBaseDetailRefs(toolName, sourceRefs) {
    ...
    for (const [index, sourceRef] of sourceRefs.slice(0, 8).entries()) {
        refs.push(createAgentDetailRef({
...
function buildPrimeSourceGraphGuidance(input) {
    ...
    const query = firstString(input.intake.extracted.queries[0], input.intake.hostIntentFrame.recognizedIntentDraft.query);
    ...
    return {
        boundary: 'Source graph guidance is code-fact evidence only; it does not backfill Recipe provenance or replace Guard, repository tests, controller acceptance, or Test-window validation.',
        recommendedQueries,
        recommendedTools: [
        ...
        sourceEvidenceRefs,
        sourceGraphRef,
        status: sourceGraphRef ? 'ready-evidence' : 'recommended',
    };
}
function buildPrimeFeedbackDigest(searchResult) {
...
function mapLifecycleIntentKind(lifecycle, hostIntentFrame) {
...
function mapPrimeSkipReason(reasonCode) {
    switch (reasonCode) {
...
function getPipeline(container) {
    try {
        return container.get('primeSearchPipeline') ?? null;
    }
    catch (err) {
        process.stderr.write(`[MCP/AgentPublicTools] primeSearchPipeline unavailable: ${err instanceof Error ? err.message : String(err)}\n`);
        return null;
    }
}
function nextIntentRef() {
    intentCounter++;
    return `intent-${Date.now().toString(36)}-${intentCounter}`;
}
function nextPrimeRef() {
    primeCounter++;
    return `prime-public-${Date.now().toString(36)}-${primeCounter}`;
}
function nextWorkRef() {
    workCounter++;
    return `work-public-${Date.now().toString(36)}-${workCounter}`;
}
function nextFinishRef() {
    finishCounter++;
    return `finish-public-${Date.now().toString(36)}-${finishCounter}`;
}
function nextGuardResultRef() {
    guardCounter++;
    return `guard-public-${Date.now().toString(36)}-${guardCounter}`;
}
function rememberIntentRecord(record) {
    INTENT_RECORDS.set(record.intentRef, record);
    if (INTENT_RECORDS.size <= 100) {
        return;
    }
    const oldest = [...INTENT_RECORDS.entries()].sort((left, right) => new Date(left[1].createdAt).getTime() - new Date(right[1].createdAt).getTime())[0]?.[0];
    if (oldest) {
