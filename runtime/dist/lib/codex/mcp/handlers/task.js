...
async function _recordDecision(ctx, args) {
    ...
    process.stderr.write('[MCP/Task] record_decision blocked: legacy local decision persistence is disabled; use alembic_decision_record with a resident Decision Register route.\n');
    return envelope({
        success: false,
        ...
        message: 'Legacy alembic_task record_decision is disabled. Use alembic_decision_record so confirmed decisions go to the Alembic durable Decision Register; no Plugin-local fake decision was written.',
        meta: { tool: 'alembic_task' },
    });
}
