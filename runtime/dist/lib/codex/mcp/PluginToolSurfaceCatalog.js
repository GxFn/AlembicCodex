...
export const TOOL_GATEWAY_MAP = Object.fromEntries(Object.entries(PLUGIN_TOOL_SURFACE_CATALOG)
    .filter(([, entry]) => entry.gateway)
    .map(([name, entry]) => [name, entry.gateway]));
export function getPluginToolSurfaceEntry(name) {
    return PLUGIN_TOOL_SURFACE_CATALOG[name] ?? null;
}
export function getPluginToolAnnotations(name) {
    return getPluginToolSurfaceEntry(name)?.annotations ?? null;
}
export function withPluginToolAnnotations(tool) {
...
export function listPluginToolSurfaceCatalog() {
