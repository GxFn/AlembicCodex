import { CODEX_AGENT_PUBLIC_TOOL_NAMES, CODEX_HOST_AGENT_WORKFLOW_TOOL_NAMES, CODEX_SOURCE_GRAPH_TOOL_NAMES, } from '../../ToolPolicy.js';
...
export function buildCodexMcpGuidance(tools) {
    const visibleToolNames = tools.map((tool) => tool.name);
    const visibleToolNameSet = new Set(visibleToolNames);
    const sourceGraphTools = visibleToolNames.filter((name) => CODEX_SOURCE_GRAPH_TOOL_NAMES.has(name));
    const sourceGraphQueryTools = sourceGraphTools.filter((name) => name !== 'alembic_source_graph_status');
...
export function buildCodexMcpInitializeInstructions(tools) {
    return buildCodexMcpGuidance(tools).instructions;
}
function buildSourceGraphPlaybookLine(sourceGraphTools, sourceGraphQueryTools) {
    ...
    const statusPrefix = sourceGraphTools.includes('alembic_source_graph_status')
        ...
        : '';
    const queryToolText = sourceGraphQueryTools.length > 0
...
function buildKnowledgePlaybookLine(knowledgeTools) {
    ...
    return `Project knowledge: use visible Recipe/knowledge tools ${formatToolList(knowledgeTools)} for standards, prior decisions, and reusable patterns.`;
}
function buildGuardPlaybookLine(guardTools) {
...
function buildLifecyclePlaybookLine(lifecycleTools) {
    ...
    return `Lifecycle: use ${formatToolList(lifecycleTools)} for intent, prime, scoped work, finish, Guard handoff, and durable decisions; source graph evidence does not replace these refs.`;
}
function buildFallbackPlaybookLine(visibleToolNameSet) {
