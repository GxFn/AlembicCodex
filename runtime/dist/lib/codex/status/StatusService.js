...
function buildCodexAutoInitStatus(projectRoot, knowledge, projectRootResolution, options = {}) {
    ...
    const skippedReason = projectRootResolution.trust !== 'trusted'
...
export function buildCodexPostInitActions(knowledge) {
    if (knowledge.usable) {
        return [
            buildCodexAgentPrimeAction({
                reason: 'Load the most relevant Alembic Recipes through the agent-facing public prime tool before non-trivial coding work.',
                startsDaemon: true,
            }),
    ...
    return [
        buildCodexHostAgentBootstrapAction({
            reason: 'Build the first Alembic project knowledge through Codex host-agent analysis; no Alembic AI Provider is required.',
            startsDaemon: true,
        }),
    ];
}
...
export function buildCodexKnowledgeGateActions(knowledge) {
    ...
    if (!knowledge.initialized) {
        actions.push(buildCodexRecommendedAction({
            ...
            reason: 'Create or attach Alembic Codex data roots according to the ProjectRegistry workspace mode.',
            startsDaemon: false,
            tool: 'alembic_codex_init',
        }));
    }
    else {
        actions.push(buildCodexHostAgentBootstrapAction({
            reason: 'Build the first Alembic project knowledge through Codex host-agent analysis; no Alembic AI Provider is required.',
            startsDaemon: true,
        }));
    }
...
export function buildCodexStatusOnboarding(input) {
    ...
    if (input.projectRootResolution && input.projectRootResolution.trust !== 'trusted') {
        return {
            ...
            summary: 'Alembic Codex cannot determine the target project directory, so project workflows cannot be used yet.',
    ...
    if (!input.knowledge.initialized) {
        const registeredStandard = input.workspace?.registered === true && input.workspace.mode === 'standard';
    ...
    if (!input.knowledge.usable) {
        return {
            ...
            nextActions: [
                buildCodexHostAgentBootstrapAction({
                    reason: 'Create the initial Alembic knowledge base by following the Mission Briefing from Codex.',
                    startsDaemon: true,
                }),
            ],
            notes: [
    ...
    if (input.hostProjectAlignment && !input.hostProjectAlignment.handoffAllowed) {
        return {
            ...
            summary: input.hostProjectAlignment.connectionState === 'mismatch'
            ...
            primaryAction: buildCodexRecommendedAction({
                label: 'Check workspace status',
                reason: 'Inspect the Codex host project, Alembic selected project, and active runtime project before Dashboard handoff.',
                startsDaemon: false,
                tool: 'alembic_codex_status',
            }),
            nextActions: [
    ...
    return {
        ...
        primaryAction: buildCodexAgentPrimeAction({
            reason: 'Load relevant Alembic Recipes through the agent-facing public prime tool before non-trivial coding work.',
            startsDaemon: !daemonReady,
        }),
        nextActions: [
            buildCodexAgentIntentAction({
                reason: 'Normalize host-declared intent first when Codex has a concrete user task and wants an intentRef.',
                startsDaemon: false,
            }),
            buildCodexAgentPrimeAction({
                reason: 'Load project conventions and trusted context from an intentRef or host-declared intent.',
                startsDaemon: !daemonReady,
            }),
            buildCodexHostAgentRescanAction({
...
function buildCodexHostProjectAlignmentNotes(alignment) {
    if (!alignment) {
        return [];
    }
...
function buildCodexRouteBoundaryNotes(enhancementRoute) {
    ...
    const providerSummary = enhancementRoute.residentDaemonJobProvider.available
        ? `${enhancementRoute.residentDaemonJobProvider.provider || 'configured'} via ${enhancementRoute.residentDaemonJobProvider.configSource || 'unknown'}`
