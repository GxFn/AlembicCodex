...
import { ALEMBIC_PLUGIN_HOST_ENV, ALEMBIC_RUNTIME_MODE_ENV, ALEMBIC_RUNTIME_MODE_PLUGIN, CODEX_ADMIN_ENABLE_ENV, CODEX_DEFAULT_MCP_TIER, CODEX_MCP_MODE_ENV, CODEX_MCP_SHIM_ENV, CODEX_PLUGIN_HOST, CODEX_PLUGIN_NAME, resolveCodexRuntimeContext, } from '../runtime/RuntimeContext.js';
...
function buildCodexRuntimeChecks(input) {
    return {
        ...
        pluginMcpEntry: input.plugin.mcp.entry.mode !== 'unknown' &&
            input.plugin.mcp.entry.mode !== 'stale-installed-cache',
        pluginSkills: input.plugin.skills.ok,
        projectRoot: !input.options.projectRootResolution ||
            input.options.projectRootResolution.trust === 'trusted',
        residentServiceContract: !input.options.residentService || input.options.residentService.status.contractVersion === 1,
    };
}
...
function buildCodexPluginMcpDiagnostics(context, registry) {
    ...
    const wrapperPath = wrapperArg
        ...
        : null;
    const wrapperSource = wrapperPath && existsSync(wrapperPath) ? readFileSync(wrapperPath, 'utf8') : '';
    const runtimeTarballPath = join(registry.plugin.root, 'runtime.tgz');
    const wrapperUsesRuntime = wrapperSource.includes('npx') &&
    ...
    const wrapperUsesStartupLock = wrapperSource.includes('lockDir') && wrapperSource.includes('npm_config_cache');
    ...
    const binary = args.find((arg) => arg === context.runtimeBin) ||
        (wrapperUsesRuntime ? context.runtimeBin : null);
    const embeddedRuntime = command === 'node' &&
        wrapperUsesRuntime &&
    ...
    const pluginHostValue = typeof registry.mcp.env?.[ALEMBIC_PLUGIN_HOST_ENV] === 'string'
        ? registry.mcp.env[ALEMBIC_PLUGIN_HOST_ENV]
        : null;
    const runtimeModeValue = typeof registry.mcp.env?.[ALEMBIC_RUNTIME_MODE_ENV] === 'string'
...
function buildCodexPluginAssetDiagnostics(registry) {
...
function buildCodexPluginManifestDiagnostics(registry) {
    return {
...
function buildCodexPluginSkillDiagnostics(registry) {
...
function buildCodexPluginReadmeDiagnostics(context, registry) {
...
function buildCodexMcpEntryDiagnostics(input) {
    const marker = readInstalledRefreshMarker(join(input.registryPluginRoot, '.alembic-dev-refresh.json'));
    ...
    const mode = staleReasons.length > 0 && configMode !== 'unknown' ? 'stale-installed-cache' : configMode;
    return {
        args: input.args,
        cacheMarker: marker,
...
function findLocalDistArg(args) {
    return (args.find((arg) => arg.endsWith('/dist/bin/codex-mcp.js') || arg.endsWith('dist/bin/codex-mcp.js')) || null);
}
function resolveLocalDistEntryPath(registryPluginRoot, marker, localDistArg) {
    if (localDistArg) {
...
function resolveCodexMcpConfigMode(localDistArg, hasWrapper) {
    if (localDistArg) {
...
function collectLocalMcpMarkerStaleReasons(input, staleReasons) {
    ...
    if (input.marker.localMcpEntry && input.localDistPath) {
        const markerEntry = resolveMaybePluginRelative(input.input.registryPluginRoot, input.marker.localMcpEntry);
...
function buildWrapperStartupLockDiagnostics(wrapperSource) {
    const configured = wrapperSource.includes('acquireStartupLock') && wrapperSource.includes('lockDir');
    ...
    return {
        ...
        waitDiagnostics: wrapperSource.includes('startup-lock-wait') &&
...
function resolveMaybePluginRelative(pluginRoot, path) {
    return path.startsWith('/') ? path : join(pluginRoot, path.replace(/^\.\//, ''));
}
function entryModeNextAction(mode) {
    switch (mode) {
...
function buildProjectRootIssues(input) {
...
function buildRuntimeCommandIssues(input) {
    const issues = [];
    ...
    if (staleCommandCwd) {
        issues.push({
            action: 'Restart Codex itself or open a new Codex session so diagnostics no longer inherit a deleted plugin cache working directory.',
...
function buildRuntimeIdentityIssues(input) {
...
function buildPluginConfigurationIssues(input) {
    const issues = [];
    if (!input.checks.packagePin) {
        issues.push({
            action: 'Update plugins/alembic-codex/.mcp.json to launch ./bin/alembic-codex-mcp-wrapper.mjs, then run npm run prepare:codex-plugin-runtime.',
    ...
    if (!input.checks.pluginMcp && input.checks.packagePin) {
        issues.push({
            action: 'Restore plugins/alembic-codex/.mcp.json Codex env defaults: ALEMBIC_RUNTIME_MODE=plugin, ALEMBIC_PLUGIN_HOST=codex, ALEMBIC_MCP_MODE=1, ALEMBIC_CODEX_MCP_MODE=1, ALEMBIC_MCP_TIER=agent, ALEMBIC_CODEX_ENABLE_ADMIN=0.',
...
function buildPluginMcpEntryIssue(entry) {
...
function buildResidentServiceContractIssues(input) {
    if (!input.checks.residentServiceContract) {
        return [
            {
                action: 'Refresh @alembic/core and the Codex plugin runtime artifact so resident service status uses the supported contract version.',
...
function buildAdminTierIssues(input) {
...
function buildResidentServiceBoundary(residentService) {
    if (!residentService) {
        return null;
    }
    const status = residentService.status;
    const localAlembicResident = status.route === 'local-alembic-daemon' && status.owner === 'alembic';
    const embeddedHostAgentRecovery = status.route === 'embedded-plugin-runtime' && status.owner === 'alembic-plugin';
    return {
