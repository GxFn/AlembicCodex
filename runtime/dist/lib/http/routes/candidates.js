...
router.post('/enrich', validate(EnrichBody), async (req, res) => {
    ...
    res.json({
        ...
        data: attachHostAgentManagedBoundary({
            ...
            unavailable: true,
            message: 'AlembicPlugin 不执行本地候选 AI 补齐；请由 Codex host agent 或 Alembic resident service 提交补齐结果。',
...
router.post('/bootstrap-refine', validate(BootstrapRefineBody), async (req, res) => {
    ...
    res.status(501).json({
        success: false,
        error: makeHostAgentManagedError('Bootstrap 候选润色不再由 AlembicPlugin 本地 AI 执行；请由 Codex host agent 或 Alembic resident service 生成 preview 后再调用 apply。'),
        data: attachHostAgentManagedBoundary({
            candidateIds,
...
router.post('/refine-preview', validate(RefinePreviewBody), async (req, res) => {
    ...
    res.status(501).json({
        success: false,
        error: makeHostAgentManagedError('候选润色预览不再由 AlembicPlugin 本地 AI 执行；请由 Codex host agent 或 Alembic resident service 生成 preview。'),
        data: attachHostAgentManagedBoundary({
            candidateId,
            before,
            after: before,
...
router.post('/refine-preview-stream', validate(RefinePreviewBody), async (req, res) => {
    ...
    res.status(501).json({
        success: false,
        error: makeHostAgentManagedError('候选流式润色预览不再由 AlembicPlugin 本地 AI 执行；请由 Codex host agent 或 Alembic resident service 生成 preview。'),
        data: attachHostAgentManagedBoundary({
            candidateId,
            before,
            after: before,
...
router.get('/refine-preview/events/:sessionId', (req, res) => {
    res.status(410).json({
        success: false,
        error: makeHostAgentManagedError(`候选润色事件流不再由 AlembicPlugin 本地 AI 执行: ${req.params.sessionId}`),
        data: attachHostAgentManagedBoundary({ sessionId: req.params.sessionId }, 'candidate-refine-preview-events'),
    });
});
/* ═══ 对话式润色 — 应用 ══════════════════════════════════ */
/**
...
router.post('/refine-apply', validate(RefineApplyBody), async (req, res) => {
    ...
    if (!parsed) {
        return void res.status(501).json({
            success: false,
            error: makeHostAgentManagedError('refine-apply 未提供 preview，AlembicPlugin 不再 fallback 调用本地 AI；请先由 Codex host agent 或 Alembic resident service 生成 preview。'),
            data: attachHostAgentManagedBoundary({
                candidateId,
                before,
                hasUserPrompt: Boolean(userPrompt?.trim()),
