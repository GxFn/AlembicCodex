...
function sendPluginExternalAiRemoved(res, feature) {
    res.status(410).json({
        success: false,
        error: {
...
router.post('/env-config', async (_req, res) => {
...
router.post('/workspace-config', async (_req, res) => {
...
router.get('/chat/events/:sessionId', async (_req, res) => {
    sendPluginExternalAiRemoved(res, 'AI 对话流事件');
});
router.post('/agent/tool', async (_req, res) => {
    sendPluginExternalAiRemoved(res, 'HTTP Agent 工具直通');
});
router.post('/agent/task', async (_req, res) => {
