...
export class AlembicResidentServiceClient {
    ...
    async searchWithResult(request) {
        ...
        if (!resolved.state?.token) {
            return createAlembicResidentServiceUnavailable(status, 'token-missing', 'Alembic resident service token is missing.', { retryable: true, telemetry: { feature } });
        }
        ...
        try {
            ...
            return createAlembicResidentServiceUnavailable(status, reason, err instanceof Error ? err.message : String(err), { retryable: true, telemetry: { endpoint: endpoint.toString(), feature } });
        }
    }
