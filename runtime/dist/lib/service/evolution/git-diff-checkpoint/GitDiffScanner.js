...
export class GitDiffScanner {
    ...
    async scanOnce(now = Date.now(), options = {}) {
        const scannedAt = new Date(now).toISOString();
        try {
            const isWorktree = (await this.#execGit(['rev-parse', '--is-inside-work-tree'], this.#projectRoot)).trim() ===
                'true';
            if (!isWorktree) {
                this.#markUnavailable(scannedAt, 'project is not a git worktree');
                return emptyResult(scannedAt);
            }
            const currentHead = normalizeHead(await this.#execGit(['rev-parse', 'HEAD'], this.#projectRoot));
            const snapshot = await this.#collectSnapshot(currentHead);
            const events = [...snapshot.eventsByKey.values()];
            const headChanged = Boolean(options.previousHead) &&
                Boolean(currentHead) &&
                options.previousHead !== currentHead;
            if (headChanged && options.previousHead && currentHead) {
                const headDiff = await this.#execGit(['diff', '--name-status', `${options.previousHead}..${currentHead}`], this.#projectRoot);
    ...
    async #collectSnapshot(currentHead) {
    ...
    async #hashUntrackedFile(hash, filePath) {
        try {
    ...
    #markUnavailable(scannedAt, message) {
        this.#status = {
            ...this.#status,
            healthy: false,
            lastError: message,
...
export function addNameStatusEvents(target, output, eventSource) {
...
function emptyResult(scannedAt) {
    return {
        dirtyPathCount: 0,
        events: [],
        head: null,
...
function isDispatchablePath(filePath) {
