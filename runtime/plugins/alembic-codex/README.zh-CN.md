...
Alembic for Codex 让 Codex 获得本地项目记忆，而不是把每一次对话都变成初始化流程。它先启动轻量 MCP shim，在不初始化数据库的情况下报告诊断和工作区状态，默认以 Ghost mode 初始化；只有在请求项目知识、Guard、Dashboard、Codex 宿主 Agent bootstrap/rescan 或显式 API AI job 时，才启动或连接当前工作区的 daemon。
...
- wrapper 会避免依赖安装写入已安装插件目录，并串行化短暂的 npx 启动/安装阶段，避免并发本地验证互相争抢同一个 `_npx` 目录。
...
`alembic_codex_bootstrap` 和 `alembic_codex_rescan` 是显式 Alembic API AI daemon job。它们需要已配置 AI Provider 凭据，并会立即返回持久 job id。Codex 重连或本地 Alembic UI 刷新后，用 `alembic_codex_job` 携带该 id 继续检查状态。

如果 Alembic daemon 在活跃 API AI job 完成前关闭或重启，下一次 daemon 生命周期会把该 job 标记为 `failed`，并记录中断原因，避免 job 永远停在 `queued` 或 `running`。需要重试时，重新启动 API AI job，或改走宿主 Agent workflow。
