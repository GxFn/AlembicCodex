...
export const CORE_CONTRACT_SPINE_ROWS = [
    {
        ...
        removalBlocker: 'No subpath removal before import scans prove no consumer or replacement path is connected.',
...
export const CORE_CONTRACT_SPINE_FIELD_POLICIES = [
    ...
    {
        cleanupTrigger: 'Remove after D24/D29 import scans and fixture replay prove no product consumer reads legacyPath/byLegacyPath.',
    ...
    {
        cleanupTrigger: 'Remove after provider and Dashboard fixture replay prove canonical acceptedEventSources replace compatibilityAliases.',
...
export const CORE_LEGACY_CONTRACT_CONVERGENCE_CANDIDATES = [
    {
        cleanupBlocker: 'Alembic and AlembicPlugin still import daemon, shared, guard, search, and core/* public subpaths; direct narrowing waits for consumer migration.',
        currentCompatibilityOwner: ['Alembic', 'AlembicPlugin', 'AlembicAgent'],
        currentConsumers: ['Alembic', 'AlembicPlugin', 'AlembicAgent'],
        decisionRationale: 'Current product source scans still consume broad public families, so D9 records them as owned Core public contracts instead of narrowing exports early.',
        id: 'D9-C01',
        legacySurface: 'Wildcard package exports for daemon, shared, guard, search, core/ast, core/capability, core/discovery, and core/enhancement families.',
        publicExposurePolicy: 'Preserved only as Core-owned deterministic public package boundaries; runtime, UI, MCP, CLI, AI-provider, and tool execution stay outside Core.',
        registryRows: ['I01', 'I03', 'I04', 'I05', 'I06', 'I07', 'I08', 'I21', 'I23'],
        removalTrigger: 'Every active product import is migrated to an explicit replacement export and consumer build or fixture replay passes.',
        replacementContract: 'D2 Core contract spine rows plus explicit package export and public API smoke coverage.',
        requiredExportPaths: [
            './daemon',
            './daemon/*',
            './guard',
    ...
    {
        cleanupBlocker: 'Dashboard and provider health fixtures still surface fileMonitor.compatibilityAliases for legacy ide-edit classification.',
        ...
        decisionRationale: 'The alias map is consumer-needed diagnostic contract data until provider and Dashboard fixtures prove canonical sources alone are sufficient.',
        id: 'D9-C02',
        legacySurface: 'fileMonitor.compatibilityAliases mapping ide-edit to host-edit.',
        publicExposurePolicy: 'Diagnostic/consumer-needed capability metadata only; canonical acceptedEventSources remain the source of truth.',
        registryRows: ['I03', 'I23'],
        removalTrigger: 'Dashboard and provider fixture replay no longer read or display compatibilityAliases and canonical event sources cover all current states.',
        replacementContract: 'Explicit file monitor capability discovery with canonical acceptedEventSources and source-contract normalizers.',
        requiredExportPaths: ['./daemon', './shared'],
    ...
    {
        cleanupBlocker: 'Alembic provider project-scope analysis, AlembicAgent evidence, Plugin IDE-agent surfaces, and Dashboard project-scope UI still consume legacyPath compatibility data.',
        ...
        decisionRationale: 'Qualified refs are mandatory for new Core-normalized paths, while legacyPath stays as compatibility input/output with explicit ambiguity behavior.',
        id: 'D9-C03',
        legacySurface: 'legacyPath, byLegacyPath, unique-legacy-path, and ambiguous-legacy-path source-ref compatibility.',
        publicExposurePolicy: 'qualifiedPath/projectScopeId are first-class; legacyPath is compatibility-only and ambiguous legacy refs must be rejected.',
        registryRows: ['I05'],
        removalTrigger: 'No current product source or fixture consumes legacyPath/byLegacyPath and qualifiedPath fixture replay passes across provider, Agent, Plugin, and Dashboard.',
        replacementContract: 'ProjectScope source refs keyed by projectScopeId and repo-qualified qualifiedPath.',
    ...
    {
        cleanupBlocker: 'No active product consumer remains after D9 alias import scan; keep BM25Scorer class but do not reintroduce BM25* type aliases.',
        currentCompatibilityOwner: [],
        currentConsumers: [],
        decisionRationale: 'Active product source scan has zero BM25Document/BM25SearchResult/BM25DocMeta imports, and Core internals now use ScorerDocument/ScorerResult/DocMeta.',
        id: 'D9-C04',
        legacySurface: 'Deprecated BM25Document, BM25SearchResult, and BM25DocMeta type aliases.',
        publicExposurePolicy: 'Deleted type aliases; current public scorer type names remain ScorerDocument, ScorerResult, and DocMeta.',
        registryRows: ['I01'],
...
export function validateCoreContractSpine(options = {}) {
...
export function validateCoreContractFieldPolicies(policies = CORE_CONTRACT_SPINE_FIELD_POLICIES) {
...
export function validateCoreLegacyContractConvergence(options = {}) {
    const expectedCandidateIds = options.expectedCandidateIds ?? CORE_LEGACY_CONVERGENCE_CANDIDATE_IDS;
    const packageExports = options.packageExports ? new Set(options.packageExports) : null;
    const sourceFiles = options.sourceFiles ? new Set(options.sourceFiles) : null;
    const activeLegacyConsumerRefs = new Set(options.activeLegacyConsumerRefs ?? []);
    const candidatesById = new Map(CORE_LEGACY_CONTRACT_CONVERGENCE_CANDIDATES.map((candidate) => [candidate.id, candidate]));
...
export function summarizeCoreContractSpine(rows = CORE_CONTRACT_SPINE_ROWS) {
...
export function summarizeCoreContractFieldPolicies(policies = CORE_CONTRACT_SPINE_FIELD_POLICIES) {
...
export function summarizeCoreLegacyContractConvergence(candidates = CORE_LEGACY_CONTRACT_CONVERGENCE_CANDIDATES) {
    const statuses = emptyLegacyConvergenceStatusCounts();
    const deletedCandidateIds = [];
    const preservedCandidateIds = [];
...
function collectRequiredFieldIssues(row, issues) {
    for (const key of [
        'capabilityCoverage',
        'capabilityDiscovery',
        'consumers',
    ...
    for (const key of ['artifactPolicy', 'driftGate', 'fixturePolicy', 'removalBlocker']) {
...
function collectLegacyConvergenceFieldIssues(candidate, issues) {
    for (const key of [
        'registryRows',
        'requiredExportPaths',
        'sourceFiles',
    ...
    if (candidate.status === 'preserved-with-owner') {
        for (const key of ['currentConsumers', 'currentCompatibilityOwner']) {
            if (candidate[key].length === 0) {
                issues.push({
                    candidateId: candidate.id,
                    code: 'missing-convergence-field',
...
function emptyFunctionClassCounts() {
    return {
...
function emptyLegacyConvergenceStatusCounts() {
    return {
        'already-solved': 0,
        blocked: 0,
        deleted: 0,
