import { CORE_FIELD_FAILURE_KINDS, isCoreFieldClass, isCoreFieldFailureKind, isCorePrivateFieldClass, } from './FieldTaxonomy.js';
...
export function validateCoreFailureTaxonomy(entries = CORE_FAILURE_TAXONOMY) {
    const issues = [];
    ...
    for (const entry of entries) {
        ...
        const expectedStableId = `core.failure.${entry.kind}`;
...
export function summarizeCoreFailureTaxonomy(entries = CORE_FAILURE_TAXONOMY) {
...
export function getCoreFailureTaxonomyEntry(kind) {
...
function emptyProblemClassCounts() {
    return {
...
function emptyRetryPolicyCounts() {
    return {
...
function emptyFailureStatusCounts() {
    return {
        blocked: 0,
        cancelled: 0,
        degraded: 0,
