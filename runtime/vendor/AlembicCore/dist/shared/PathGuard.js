...
class PathGuard {
    ...
    #excludeReason = '';
    /** 宿主显式扩展的项目内写入前缀 */
    #extraProjectWritePrefixes = new Set();
    /** 宿主显式扩展的项目根可写文件 */
    #extraProjectWritableFiles = new Set();
    /**
     ...
     */
    configure({ projectRoot, packageRoot, knowledgeBaseDir, extraAllowPaths = [], extraProjectWritePrefixes = [], extraProjectWritableFiles = [], }) {
    ...
    assertProjectWriteSafe(targetPath) {
        ...
        if (this.#isExcludedProject) {
            if (firstSegment === '.asd') {
                throw new PathGuardError(resolved, projectRoot, `排除项目保护 (${this.#excludeReason}): 禁止创建 .asd/ 运行时数据`);
            }
            const kbDir = this.#resolveKnowledgeBaseDir();
            if (kbDir && firstSegment === kbDir) {
                throw new PathGuardError(resolved, projectRoot, `排除项目保护 (${this.#excludeReason}): 禁止创建 ${kbDir}/ 知识库数据`);
            }
            ...
            throw new PathGuardError(resolved, projectRoot, `排除项目保护 (${this.#excludeReason}): "${relative}" 不在允许范围内`);
        }
        // 检查是否在允许的前缀中
        ...
        // 不在允许的写入范围内
        throw new PathGuardError(resolved, projectRoot, `项目内写入范围受限: "${relative}" 不在允许的目录中（允许: ${[...this.#projectWriteScopePrefixes(), kbDir || 'Alembic'].join(', ')}）`);
    }
    /** 安全检查（不抛错，返回 boolean） */
    isSafe(targetPath) {
        try {
    ...
    addProjectWritePrefix(relativePrefix) {
...
function normalizeProjectWritePrefix(value) {
    return normalizeProjectRelativePath(value).split(path.sep)[0] || '';
}
function normalizeProjectRootFile(value) {
