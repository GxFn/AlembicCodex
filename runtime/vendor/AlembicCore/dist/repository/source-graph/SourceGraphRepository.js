...
import { createSourceFileNode, createSourceGraphEdge, createSourceGraphFreshness, createSourceGraphSnapshot, createSourceSymbolNode, } from '../../domain/source-graph/index.js';
import { sourceGraphEdges, sourceGraphFiles, sourceGraphGenerations, sourceGraphSymbols, } from '../../infrastructure/database/drizzle/schema.js';
...
export class SourceGraphRepositoryImpl extends RepositoryBase {
    ...
    async completeGeneration(generationId, updates = {}) {
    ...
    async replaceGeneration(input) {
        ...
        const preparedFiles = (input.files ?? []).map((file) => createSourceFileNode({
            ...file,
            generationId: preparedSnapshot.generationId,
            projectRoot: preparedSnapshot.projectRoot,
        }));
        const preparedSymbols = (input.symbols ?? []).map((symbol) => createSourceSymbolNode({
            ...symbol,
            generationId: preparedSnapshot.generationId,
        }));
        const preparedEdges = (input.edges ?? []).map((edge) => createSourceGraphEdge({
            ...edge,
            generationId: preparedSnapshot.generationId,
        }));
    ...
    async getLatestSnapshot(projectRoot, repoId = 'default') {
        const rows = this.drizzle
            .select()
            .from(this.table)
    ...
    async upsertFile(input, refreshStats = true) {
    ...
    async upsertSymbol(input, refreshStats = true) {
    ...
    async upsertEdge(input, refreshStats = true) {
    ...
    async findFile(generationId, repoRelativePath) {
        const rows = this.drizzle
            .select()
            .from(sourceGraphFiles)
            .where(and(eq(sourceGraphFiles.generationId, generationId), eq(sourceGraphFiles.repoRelativePath, repoRelativePath)))
            .limit(1)
            .all();
        return rows.length > 0 ? mapFileRow(rows[0]) : null;
    }
    async listFiles(generationId) {
        const rows = this.drizzle
            .select()
            .from(sourceGraphFiles)
    ...
    async getSymbol(generationId, symbolId) {
        const rows = this.drizzle
            .select()
            .from(sourceGraphSymbols)
            .where(and(eq(sourceGraphSymbols.generationId, generationId), eq(sourceGraphSymbols.symbolId, symbolId)))
            .limit(1)
            .all();
        return rows.length > 0 ? mapSymbolRow(rows[0]) : null;
    }
    async listSymbols(generationId) {
        const rows = this.drizzle
            .select()
            .from(sourceGraphSymbols)
    ...
    async searchSymbols(generationId, query, options = {}) {
        const conditions = [eq(sourceGraphSymbols.generationId, generationId)];
        const trimmedQuery = query.trim();
        if (trimmedQuery !== '') {
            const search = `%${trimmedQuery}%`;
            const searchCondition = or(like(sourceGraphSymbols.displayName, search), like(sourceGraphSymbols.qualifiedName, search), like(sourceGraphSymbols.symbolId, search));
            if (searchCondition) {
    ...
    async listEdges(generationId, options = {}) {
        const conditions = [eq(sourceGraphEdges.generationId, generationId)];
        if (options.kind) {
        ...
        if (options.filePath) {
            const fileCondition = or(eq(sourceGraphEdges.fromFilePath, options.filePath), eq(sourceGraphEdges.toFilePath, options.filePath), eq(sourceGraphEdges.siteFilePath, options.filePath));
            if (fileCondition) {
    ...
    async findEdgesForSymbol(generationId, symbolId, direction = 'both') {
        const conditions = [eq(sourceGraphEdges.generationId, generationId)];
        if (direction === 'incoming') {
            ...
            const symbolCondition = or(eq(sourceGraphEdges.fromSymbolId, symbolId), eq(sourceGraphEdges.toSymbolId, symbolId));
            if (symbolCondition) {
    ...
    async findEdgesForFile(generationId, filePath) {
        return this.listEdges(generationId, { filePath, limit: 500 });
    }
    async getStats(generationId) {
        const snapshot = (await this.refreshGenerationStats(generationId)) ??
    ...
    async clearGeneration(generationId) {
        const files = countRows(this.drizzle
            .select({ cnt: count() })
            .from(sourceGraphFiles)
            .where(eq(sourceGraphFiles.generationId, generationId))
            .all());
        const symbols = countRows(this.drizzle
            .select({ cnt: count() })
            .from(sourceGraphSymbols)
            .where(eq(sourceGraphSymbols.generationId, generationId))
            .all());
        const edges = countRows(this.drizzle
            .select({ cnt: count() })
            .from(sourceGraphEdges)
    ...
    async refreshGenerationStats(generationId) {
        ...
        const fileRows = this.drizzle
            ...
            .where(eq(sourceGraphFiles.generationId, generationId))
            .all();
        const symbolCount = countRows(this.drizzle
            .select({ cnt: count() })
            .from(sourceGraphSymbols)
            .where(eq(sourceGraphSymbols.generationId, generationId))
            .all());
        const edgeCount = countRows(this.drizzle
            .select({ cnt: count() })
            .from(sourceGraphEdges)
            .where(eq(sourceGraphEdges.generationId, generationId))
            .all());
        const languageCoverage = Array.from(new Set(fileRows.map((row) => row.language).filter((language) => language.trim() !== ''))).sort();
        const parseErrorCount = fileRows.reduce((total, row) => total + parseJsonArray(row.parseErrorsJson).length, 0);
        const now = Date.now();
        this.drizzle
            .update(this.table)
            .set({
    ...
    async resolveProjectRoot(generationId, explicitProjectRoot) {
    ...
    async getRequiredSnapshot(generationId) {
        const snapshot = await this.getSnapshot(generationId);
        if (!snapshot) {
    ...
    async getRequiredFile(generationId, repoRelativePath) {
    ...
    async getRequiredSymbol(generationId, symbolId) {
    ...
    async getRequiredEdge(generationId, edgeId) {
        const rows = this.drizzle
            .select()
            .from(sourceGraphEdges)
            .where(and(eq(sourceGraphEdges.generationId, generationId), eq(sourceGraphEdges.edgeId, edgeId)))
            .limit(1)
            .all();
        if (rows.length === 0) {
...
function mapGenerationRow(row) {
    return createSourceGraphSnapshot({
        ...
        languageCoverage: parseJsonArray(row.languageCoverageJson),
...
function mapFileRow(row) {
    return createSourceFileNode({
        ...
        parseErrors: parseJsonArray(row.parseErrorsJson),
...
function mapSymbolRow(row) {
    return createSourceSymbolNode({
        ...
        selectionRange: mapOptionalRange(row.selectionStartLine, row.selectionStartColumn, row.selectionEndLine, row.selectionEndColumn),
...
function mapEdgeRow(row) {
    return createSourceGraphEdge({
        ...
        site: mapOptionalRange(row.siteStartLine, row.siteStartColumn, row.siteEndLine, row.siteEndColumn),
...
function mapOptionalRange(startLine, startColumn, endLine, endColumn) {
