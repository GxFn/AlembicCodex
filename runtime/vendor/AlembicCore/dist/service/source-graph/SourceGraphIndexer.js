...
import { createSourceGraphDiagnostic, createSourceGraphFreshness, createSourceGraphStatusResult, } from '../../domain/source-graph/index.js';
...
export class SourceGraphIndexer {
    ...
    async buildIncremental(input) {
        ...
        const changedFiles = normalizeRepoPathList([...(input.changedFiles ?? []), ...detected.changedFiles], options.projectRoot).filter((repoPath) => currentByPath.has(repoPath));
        const deletedFiles = normalizeRepoPathList([...(input.deletedFiles ?? []), ...detected.deletedFiles], options.projectRoot);
        const changedSet = new Set(changedFiles);
        const deletedSet = new Set(deletedFiles);
        const impacted = new Set([...changedSet, ...deletedSet]);
        const preservedFiles = baseFiles
            .filter((file) => !impacted.has(file.repoRelativePath) && currentByPath.has(file.repoRelativePath))
    ...
    async buildGeneration(input) {
        ...
        const parsedFiles = await Promise.all(input.inventory.map((file) => parseInventoryFile(file, input.options, input.generationId, knownPaths)));
...
function normalizeIndexOptions(input) {
    ...
    return {
        ...
        includeExtensions: new Set((input.includeExtensions ?? DEFAULT_INCLUDE_EXTENSIONS).map(normalizeExtension)),
...
async function walkDirectory(directory, options, files) {
    let entries;
    try {
...
async function parseInventoryFile(file, options, generationId, knownPaths) {
    ...
    if (file.sizeBytes > options.maxFileSizeBytes) {
        return skippedFile(baseFile, 'large-file-skipped', 'File exceeded source graph index size limit.');
    }
    if (!PARSABLE_EXTENSIONS.has(file.extension)) {
        return skippedFile(baseFile, 'unsupported-language', `Unsupported source graph language: ${file.language}.`);
    }
...
function skippedFile(file, code, message) {
    return {
        file: {
            ...file,
            parseStatus: 'skipped',
...
function partialFile(file, code, message) {
    return {
        file: {
            ...file,
            parseStatus: 'partial',
...
function failedFile(file, message) {
    return {
        file: {
            ...file,
            parseStatus: 'failed',
...
function extractSymbols(content, file, generationId, extractorVersion, lineCount) {
    const symbols = [
        moduleSymbolFromInventory(file, generationId, extractorVersion, lineCount),
    ];
    const lines = content.split(/\r\n|\n|\r/);
    const symbolPattern = /\b(export\s+)?(?:abstract\s+)?(class|interface|enum|function|type|const|let|var)\s+([A-Za-z_$][\w$]*)/g;
...
function moduleSymbolFromInventory(file, generationId, extractorVersion, lineCount) {
    return {
        generationId,
...
function moduleSymbol(file, lineCount) {
    return {
...
function extractImportEdges(content, file, generationId, knownPaths) {
    const lines = content.split(/\r\n|\n|\r/);
    const edges = [];
    const importPattern = /\bimport\s+(?:type\s+)?(?:[^'"()]*?\s+from\s+)?['"]([^'"]+)['"]|\brequire\(\s*['"]([^'"]+)['"]\s*\)/g;
    for (const [index, line] of lines.entries()) {
...
async function detectChangedFiles(options, baseFiles, currentByPath) {
...
function createFreshness(status, generationId, now, diagnostics) {
    const freshnessStatus = status === 'indexed' ? 'fresh' : status === 'partial' ? 'partial' : 'degraded';
...
function chooseSnapshotStatus(files, diagnostics) {
...
function summarizeDegradedReason(diagnostics) {
    if (diagnostics.length === 0) {
        return undefined;
    }
    return Array.from(new Set(diagnostics.map((diagnostic) => diagnostic.code))).join(',');
}
function resolveRelativeImport(currentFile, specifier, knownPaths) {
    if (!specifier.startsWith('.')) {
        return undefined;
    }
    const base = normalizeRepoRelative(path.posix.normalize(path.posix.join(path.posix.dirname(currentFile), specifier)));
    const candidates = [
        base,
...
function edgeTouchesFiles(edge, impacted) {
    return ((edge.fromFilePath !== undefined && impacted.has(edge.fromFilePath)) ||
        (edge.toFilePath !== undefined && impacted.has(edge.toFilePath)) ||
        (edge.siteFilePath !== undefined && impacted.has(edge.siteFilePath)));
}
function normalizeRepoPathList(paths, projectRoot) {
    return Array.from(new Set(paths.map((item) => normalizeInputPath(item, projectRoot)))).sort();
}
function normalizeInputPath(input, projectRoot) {
...
function classificationForPath(filePath) {
...
function symbolKindForDeclaration(kind) {
    switch (kind) {
        case 'class':
            return 'class';
        case 'interface':
...
function createGenerationId(repoId, now) {
