...
async function persistKeyFindings({ ctx, session, dimensionId, keyFindings, }) {
    try {
        ...
        for (const finding of keyFindings) {
            await knowledgeGraphService.addEdge(dimensionId, 'dimension', finding.substring(0, 80), 'finding', 'discovered_in', { source: 'host-agent-bootstrap', sessionId: session.id });
        }
    }
    catch (err) {
