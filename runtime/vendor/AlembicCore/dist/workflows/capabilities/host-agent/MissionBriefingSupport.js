...
export const DEFAULT_BRIEFING_PROFILE = 'cold-start-host-agent';
...
export function createBriefingPlan(input = {}) {
    ...
    if (profile === 'cold-start-host-agent' && input.rescan) {
        throw new Error('[MissionBriefing] cold-start-host-agent profile cannot accept rescan evidence');
    }
    return {
        profile,
        rescan: input.rescan,
