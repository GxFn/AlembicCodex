...
class SnapshotEpisodicMemory {
    #data;
    constructor(data) {
        this.#data = data;
    }
    static fromJSON(value) {
        return new SnapshotEpisodicMemory(value && typeof value === 'object' ? value : {});
    }
    getCompletedDimensions() {
        const completed = this.#data.completedDimensions;
        if (Array.isArray(completed)) {
            return completed.filter((item) => typeof item === 'string');
        }
    ...
    getDimensionReport(dimId) {
        ...
        const report = reports[dimId];
        if (!report || typeof report !== 'object') {
            return null;
        }
        const referencedFiles = report.referencedFiles;
        return {
            referencedFiles: Array.isArray(referencedFiles)
                ? referencedFiles.filter((item) => typeof item === 'string')
                : [],
        };
    }
    toJSON() {
